...
export const BalanceAdjustmentsPage: React.FC = () => {
  ...
  const [targetAdjustment, setTargetAdjustment] = useState<BalanceAdjustment | null>(null);
  ...
  const openReversal = (adj: BalanceAdjustment) => {
    setTargetAdjustment(adj);
    setReversalReason('');
    setShowReversalModal(true);
  };

  const handleConfirmReversal = () => {
    if (!currentUser || !targetAdjustment) return;
    setFeedback(null);

    const res = organizationService.reverseBalanceAdjustment(
      currentUser,
      targetAdjustment.id,
      reversalReason
    );
    ...
    if (!res.success) {
      setFeedback({ type: 'error', message: res.error || 'Failed to reverse adjustment.' });
      return;
    }

    setFeedback({
      type: 'success',
      message: `Adjustment ${targetAdjustment.adjustmentReference} reversed successfully via ${res.reversalAdjustment?.adjustmentReference}.`,
    });
  ...
  return (
    <div className="space-y-6 pb-12">
                  ...
                  <tr key={adj.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      ...
                      >
                        ...
                        <span>
                          {adj.direction === 'Increase' ? '+' : '-'}ZMW{' '}
                          {adj.amount.toLocaleString('en-ZM', { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 font-mono text-[11px]">
                      <span className="text-slate-400">
                        {adj.previousBalance.toLocaleString('en-ZM', { minimumFractionDigits: 2 })}
                      </span>
                      <span className="text-slate-300 mx-1">→</span>
                      <span className="font-semibold text-slate-800">
                        {adj.newBalance.toLocaleString('en-ZM', { minimumFractionDigits: 2 })}
                      </span>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {adj.status === 'Completed' && !adj.reversedAdjustmentId && (
                          <button
                            onClick={() => openReversal(adj)}
      ...
      {/* ========================================== */}
      {showReversalModal && targetAdjustment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-lg">Reverse Adjustment</h3>
                <p className="text-xs text-slate-500">
                  {targetAdjustment.adjustmentReference} • ZMW {targetAdjustment.amount.toFixed(2)}
                </p>
              </div>
            </div>

            <p className="text-xs text-slate-600 leading-relaxed">
              This will create an equal and opposite compensating balance adjustment in the ledger to restore the staff member's balance. The original record remains immutable in the audit trail.
            </p>
            ...
            <div>
              ...
              <textarea
                rows={2}
                required
                placeholder="e.g. Accidental amount transposition, duplicate entry, supervisor correction..."
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-2 text-xs text-white bg-amber-600 hover:bg-amber-700 disabled:opacity-50 rounded-lg font-semibold shadow-xs"
              >
                Confirm Reversal
              </button>
            </div>
